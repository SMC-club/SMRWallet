...
void WalletMain::setOnline(bool pbFlag, const QString& psReason)
{
    ...
    this->setWindowTitle(QString("SMR Wallet [%1%2]").arg(strAccountID).arg(pbFlag ? "" : ", offline"));
...
void WalletMain::socketConnect()
{
    ...
    vector<QString> servers = {"ws://188.166.133.238:44122", "ws://167.99.39.220:44122/", "ws://188.166.119.125:44122/"};
...
void WalletMain::submitResponse(const QJsonObject& poResp)
{
    ...
    if (result["status"] == "error")
        Show("Transaction error", QString("Failure while committing transaction to the SMR network: ") + poResp["error_message"].toString(), E_WARN);
    else if (result["engine_result"].toString() != "tesSUCCESS")
        Show("Transaction error", QString("Error while processing transaction by the SMR network: ") + result["engine_result_message"].toString(), E_WARN);
    else
    ...
    if (eNone == eRes)
    {
        ...
        if (pbJustAsk)
        {
            QString strConf = "I'm about to send " + ui->amountToSend->text()
                    + " SMR to " + ui->receiverAddressEdit->text() + ". Do you agree?";
...
void WalletMain::on_actionProxy_triggered()
{
    ...
    if (proxysettings.exec() == QDialog::Accepted)
    {
        ...
        SMRSettings.Write(CIniworker::W_PROXY);
        doReconnect();
    };
}
...
{
    SMRSettings.Read(CIniworker::R_PROXY);
